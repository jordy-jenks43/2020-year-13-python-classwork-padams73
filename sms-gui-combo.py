...
def class_count():
    ''' Count number of students in selected class, displaying number in Text label. '''
    
    count = 0
    for student in student_list:
        if class_combo.value in student.get_classes():
            count += 1
    count_label.value = "Total students: " + str(count)
...
# Combo box contains list of all classes
class_combo = Combo(app,options=["GRA", "BIO", "PHY", "MAT", "DTC", "ART", "ENG", "XYZ"], command=class_count)

# Label to display number of students in selected class
